...
- **Developed and functionally verified on macOS, and separately on Linux.** `npm run dev`, `npm run build`, the full `npm test` suite (304 tests: 192 unit + 112 e2e), and both `npm run package:mac` and `package:mac:dist` were actually run on macOS, including launching the packaged `.app` straight off the built `.dmg`. `npm run package:win:dist` was also run there (cross-packaged from macOS — see "Packaging") and produced a valid NSIS installer and portable `.exe`, but neither was *executed*, since no Windows machine was available in that environment. The Windows-specific code paths (accelerator strings, `%LOCALAPPDATA%`-style paths via `app.getPath`) use the same cross-platform Electron APIs exercised on macOS and are expected to work, but that's an expectation, not a verified fact, until someone runs the installer on real Windows. The archive-integrity-hashing, app-icon, CI-audit, and auto-update work above was separately built and the full suite (same count, growing alongside it) run again on ARM64 Linux each time, including a real X display for the e2e Electron windows.
...
3. **Parallel crawling** for `.sitearchive` site captures (the concurrency setting is plumbed through but the crawl is currently sequential — see Known Limitations).
4. **Search inside a `.sitearchive`**, using the `index.sqlite` catalog and the per-page extracted text that already ship in the container.
5. **Resume-only retry** for failed pages, instead of re-running the whole capture.
6. **Full-text search ranking/snippets** in the Library (currently exact FTS5 match, no relevance snippet preview).
7. **True multi-window support.**
8. **Export/import of the whole library** (not just per-archive export), for migrating between machines.
9. **Accessibility pass** on the trusted UI (keyboard navigation, screen reader labeling).

...
As of this build: **218 unit tests** and **122 end-to-end tests** (see Known Limitations for which of the most recent additions have and haven't been reconfirmed passing in this specific environment).
...
3. **"Save Forum"**: a forum-aware capture mode on top of the existing `.sitearchive` machinery, not a parallel system. User picks a scope before capturing — **current thread** (every page of that thread only), **current section** (its pagination plus every thread listed in it), or **entire forum** (every public section and thread, subject to limits) — and the crawler's existing scope/frontier logic gets forum-specific link classification to match: follow thread pagination and next-page links (numbered pages, "next", `page=`/`start=` offsets), but never sign-in/register/account/moderation links, never reply/quote/edit/delete/report/react links or any non-GET action, never calendars/search/tag-filter combinations, and never links outside the selected scope — those still get preserved as clearly-marked external/uncaptured links (rewritten offline links must never imply a page was captured when it wasn't). Posts, authors, timestamps, post numbers, avatars, emoji, thumbnails, and accessible attachments are captured and deduplicated the same way assets already are; per-post anchors are preserved so a link (and search result) can jump straight to the right post offline. Browses the user's existing signed-in session to reach authorized content, but — like every capture already does — never writes cookies, tokens, session ids, or credentials into the portable archive, and warns before capturing private/account-only content. Capture controls (scope, max pages/depth/size, include-profiles, include-attachments, external-image fetching, request delay) extend `CaptureScopeDialog`/`CaptureScope` rather than a new dialog; progress/manifest/pause-resume-cancel reuse `CaptureProgressDialog`, the checkpoint/resume journal, and the existing "never claim complete when it isn't" manifest fields. Extracted post text feeds the existing `pages_fts` search exactly like any other captured page, additionally searchable by author and section. Primary real-world test target: a small, controlled capture against `https://www.rangerovers.net/forums/` (public index → one section → one multi-page thread, respecting `robots.txt`, a conservative rate, and no sign-in/account actions) — used for manual/optional verification only, never as a required automated-test dependency; automated tests run against local fixtures built from the structures found there.

...
4. **Dependency audit in CI** (`npm audit` / `osv-scanner`) and a documented update cadence for Electron itself.
5. **Parallel crawling** for `.sitearchive` site captures (the concurrency setting is plumbed through but the crawl is currently sequential — see Known Limitations).
6. **Search inside a `.sitearchive`**, using the `index.sqlite` catalog and the per-page extracted text that already ship in the container.
7. **Resume-only retry** for failed pages, instead of re-running the whole capture.
8. **Full-text search ranking/snippets** in the Library (currently exact FTS5 match, no relevance snippet preview).
9. **True multi-window support.**
10. **Export/import of the whole library** (not just per-archive export), for migrating between machines.
11. **Accessibility pass** on the trusted UI (keyboard navigation, screen reader labeling).

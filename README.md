...
2. **Packaged app icons** and proper `build/` resources for macOS/Windows.
3. **Code signing + notarization** for macOS, code signing for Windows, so builds can be distributed publicly without OS warnings.
4. **Auto-update** via `electron-updater`, pointed at a release channel once signed builds exist.
5. **Dependency audit in CI** (`npm audit` / `osv-scanner`) and a documented update cadence for Electron itself.
6. **Parallel crawling** for `.sitearchive` site captures (the concurrency setting is plumbed through but the crawl is currently sequential — see Known Limitations).
7. **Search inside a `.sitearchive`**, using the `index.sqlite` catalog and the per-page extracted text that already ship in the container.
8. **Resume-only retry** for failed pages, instead of re-running the whole capture.
9. **Full-text search ranking/snippets** in the Library (currently exact FTS5 match, no relevance snippet preview).
10. **True multi-window support.**

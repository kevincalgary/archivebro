...
3. **Search inside a `.sitearchive`**, using the `index.sqlite` catalog and the per-page extracted text that already ship in the container.
4. **Resume-only retry** for failed pages, instead of re-running the whole capture.
5. **Full-text search ranking/snippets** in the Library (currently exact FTS5 match, no relevance snippet preview).
6. **True multi-window support.**
7. **Export/import of the whole library** (not just per-archive export), for migrating between machines.
8. **Accessibility pass** on the trusted UI (keyboard navigation, screen reader labeling).

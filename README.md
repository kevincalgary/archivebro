...
3. **Resume-only retry** for failed pages, instead of re-running the whole capture.
4. **Full-text search ranking/snippets** in the Library (currently exact FTS5 match, no relevance snippet preview).
5. **True multi-window support.**
6. **Export/import of the whole library** (not just per-archive export), for migrating between machines.
7. **Accessibility pass** on the trusted UI (keyboard navigation, screen reader labeling).
